...
namespace lpzrobots{
  ...
  void EPuckBluetooth::initConnection(char* port){
    int error = 0;
    comm=new SerialComm();
  ...
  int EPuckBluetooth::getSensors(sensor* sensors, int _sensorCount){
	...
	if(sensors[i]<0)sensors[i]=0;
      ...
      if(conf.MIC_STATE&& i>=numOfSensor.MIC0&&i<=(numOfSensor.MIC2+100) )sensors[i]=arrSensor[i];
      if(conf.CAM_STATE&& i>=numOfSensor.CAM&&i<=(numOfSensor.CAM+camPixNum) )sensors[i]=arrSensor[i];
    }
    return sensorCount;
  }//end getSensors
