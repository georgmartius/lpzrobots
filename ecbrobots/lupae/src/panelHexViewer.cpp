...
QPanelHexViewer::QPanelHexViewer(QWidget *parent) : QWidget(parent)
{
...
bool QPanelHexViewer::hasPage(ushort iPageNumber, ushort iPageSize_in_Bytes)
{
  ...
  try
  {
    if (startIndex < pgmBuffer.length()) return true;
  }
  catch (...)
  {}
  return false;
  ...
  if (!pgmBuffer.isEmpty())
  {
    ...
    for (int j = 0; j < lines; j++)
    {
      ...
      for (int i = 0; i < 16; i++)
      {
        ...
        if (index >= pgmBuffer.length()) break;
        if (i == 0)
        {
        ...
        if (i == 7) sPgm += "  "; else sPgm += " ";
        if (31 < (QByte)pgmBuffer[index] && (QByte)pgmBuffer[index] < 127) sAscii += pgmBuffer[index]; else sAscii += ".";
      }
      painter.drawText(QRect(10, 10 + j * lineSpacing, 50, lineSpacing), Qt::AlignRight, sSpace);
      painter.drawText(QRect(80, 10 + j * lineSpacing, 370, lineSpacing), Qt::AlignLeft, sPgm);
      painter.drawText(QRect(430, 10 + j * lineSpacing, 200, lineSpacing), Qt::AlignLeft, sAscii);
    }
  }
}
