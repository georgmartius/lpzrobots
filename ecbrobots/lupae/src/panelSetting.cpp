...
QPanelSetting::QPanelSetting()
{
  ...
  labelPixmap_USB_ISP_Adapter->setPixmap(QPixmap::fromImage(QImage(":/images/USB_ISP_Adapter.png").scaledToWidth(200,
  ...
  labelPixmap_USB_UART_Adapter->setPixmap(QPixmap::fromImage(QImage(":/images/USB_UART_Adapter.png").scaledToWidth(200,
      Qt::SmoothTransformation)));
  ...
  labelPixmap_USB_XBEE_Adapter->setPixmap(QPixmap::fromImage(QImage(":/images/USB_XBEE_Adapter.png").scaledToWidth(200,
      Qt::SmoothTransformation)));
...
void QPanelSetting::USBDevice_Name_changed(QString elementName)
{
  ...
  if (signaling) emit signal_USBDeviceName_changed(elementName);
}
...
void QPanelSetting::USBDevice_Baudrate_changed(QString elementName)
{
  ...
  if (signaling) emit signal_USBDeviceBaudrate_changed(sList.at(0));
}
...
void QPanelSetting::setUSBDeviceBaudrate(QString baudrate)
{
  ...
  {
    QString sLine = coBox_USBDeviceBaudrate->itemText(i).split(" ").at(0);
    if(sLine.compare(baudrate) == 0) coBox_USBDeviceBaudrate->setCurrentIndex(i);
  }
...
{
  if (signaling) emit signal_AVRDeviceAccessSpeed_changed(elementName);
}
...
{
  if (signaling) emit signal_USBDeviceXBeeType_changed(elementName);
}
void QPanelSetting::setUSBDeviceXBeeType(QStringList connectionList)
{
...
{
  if (signaling) emit signal_XBeeRemoteNodeIdentifier_changed(elementName);
}
...
//------------------------------------------------------------------------------------------------------
void QPanelSetting::setTargetDeviceName(QString text)
{
