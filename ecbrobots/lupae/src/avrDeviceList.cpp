...
AVRDEVICE* QAVR_DeviceList::getDevice(uint Signature)
{
  ...
  while(tmp != 0)
  {
    if(tmp->Signature == Signature) return tmp;
    tmp = tmp->next;
  }
  return 0;
}
...
bool QAVR_DeviceList::startElement(const QString&, const QString&, const QString &name, const QXmlAttributes &attrs)
{
  ...
  try
  {
    ...
    if(name=="AVR_Device")
    {
      ...
      s = attrs.value("DeviceName");    avrDevice->Name        = new QString(s.toAscii());
      s = attrs.value("Signature");     avrDevice->Signature     = s.toUInt(&ok, 16);
      s = attrs.value("PageSize");     avrDevice->PageSizeBytes  = s.toUShort();
      s = attrs.value("NumberPages");   avrDevice->NumberPages     = s.toUShort();
      s = attrs.value("NumberFuseBytes"); avrDevice->NumberFuseBytes  = (QByte)s.toUShort()&0x03;
    }
    if(name=="FuseBitsLow")
    {
      currentFuseByte = 0;
