...
namespace lpzrobots
{
  ...
  ECBManager::ECBManager() :
    Configurable("ECBManager", "$ID$"), simulation_time_reached(false), commInitialized(false)
  {
    globalData.configs.push_back(this);
    
  }

  ECBManager::ECBManager(ECBCommunicator* comm) :
    Configurable("ECBManager", "$ID$"), simulation_time_reached(false), comm(comm), commInitialized(true)
  {
    globalData.configs.push_back(this);
  ...
  bool ECBManager::run(int argc, char** argv)
  {
    ...
    if (globalData.debug)
      cout << "ECBManager: initialising ECBCommunicator..." << endl;
    ...
    if (globalData.debug)
      cout << "ECBManager: calling start function..." << endl;
    start(globalData);

    if (globalData.debug)
      cout << "ECBManager: handling console parameters..." << endl;
    handleStartParameters(argc, argv);

    // init console...defined in console.h
    if (globalData.debug)
      cout << "ECBManager: initialising console..." << endl;
    initializeConsole();

    initConsole();

    if (globalData.debug)
      cout << "ECBManager: starting communication..." << endl;
    globalData.comm->start();

    // init the PlotOptionEngine
    if (globalData.debug)
      cout << "ECBManager: starting the PlotOptionEngine..." << endl;

    // run the loop
    if (globalData.debug)
      cout << "ECBManager: starting the loop..." << endl;
    ...
    if (globalData.debug)
      cout << "ECBManager: loop finished." << endl;

    // close all things which where used
    if (globalData.debug)
      cout << "ECBManager: closing console..." << endl;
    closeConsole();

    if (globalData.debug)
      cout << "ECBManager: Reaching the final end. Bye!" << endl;
