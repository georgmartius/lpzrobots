...
PiMax::PiMax(const PiMaxConf& conf_)
  : AbstractController("PiMax", "0.1"), 
    conf(conf_), t(0) 
{

  addParameterDef("metrics", &useMetric, true, "whether to use metrics in gradient space");
  addParameterDef("epsC", &epsC, 0.01,    0,5, "learning rate of the controller");
...
Matrix PiMax::pseudoInvL(const Matrix& L, const Matrix& A, const Matrix& C){
  if(pseudo == 0){
...
void PiMax::learn(){
  ...
  // the effective s/a is (actual-steps4delay) element of buffer  
  int s4delay = ::clip(conf.steps4Delay,1,buffersize-1);
  const Matrix& s       =  s_buffer[(t - max(s4delay,1) + buffersize) % buffersize];
  const Matrix& a_creat =  a_buffer[(t - max(s4delay,1) + buffersize) % buffersize];
  const Matrix& s_fut   =  s_buffer[t% buffersize]; // future sensor (with respect to s,a)
  const Matrix& xi_1    = xi_buffer[(t - 1 + buffersize) % buffersize]; 
  const Matrix& L_1     =  L_buffer[(t - 1 + buffersize) % buffersize]; 
  ...
  L                     = A * (C & g_prime) + S;
  L_buffer[t % buffersize] = L; 
  
  R = A * C+S; // this is only used for visualization

  const Matrix& ds      = L_1 * xi_1 + xi;
  Sigma                += (ds.multMT()-Sigma)*epsSigma;
  const Matrix& dmu     = ((A^T)*Sigma.pseudoInverse() * ds) & g_prime;
  //  const Matrix& dmu     = ((A^T)*Sigma.pseudoInverse() * ds);

  const Matrix& epsrel  = (C*ds) & dmu * 2 * sense;
  
  const Matrix& metric = useMetric ? g_prime.map(one_over).map(sqr) : g_prime.mapP(1, constant);
  ...
  if(epsC > 0){
    C += ((( dmu * (ds^T) - (epsrel & a) * (s^T)) & metric) * epsC
                                                       ).mapP(.05, clip); 
    if(damping)
      C += (((C_native-C).map(power3))*damping         ).mapP(.05, clip);
    h += ((((epsrel & a)) & metric) * (-epsC)          ).mapP(.05, clip);
    
    if(intern_isTeaching && gamma > 0){    
      // scale of the additional terms
      const Matrix& metric = (A^T) * L.multTM() * A;
      
      const Matrix& a      = a_buffer[(t-1)% buffersize];
      const Matrix& xsi    = a_teaching - a;
      const Matrix& delta  = xsi.multrowwise(g_prime);
      C += ((metric * delta*(s^T) ) * (gamma * epsC)).mapP(.05, clip);
