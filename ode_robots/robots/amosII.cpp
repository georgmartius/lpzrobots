...
namespace lpzrobots {
    ...
    // the constructor
    AmosII::AmosII(const OdeHandle& odeHandle, const OsgHandle& osgHandle,
    ...
    */
    ...
    void AmosII::create( const osg::Matrix& pose ){
        ...
        if (conf.useBack) {
            ...
            HingeJoint* k = new HingeJoint(front, center,
                    nullpos * TRANSM(-conf.frontLength/2,0,0) * frontPos,
                    axis);
            k->init(odeHandle, osgHandleJoint, true, conf.width * 1.05);
            joints.push_back(k);
            // parameters are set later
            OneAxisServo* servo = new OneAxisServoVel(
                    odeHandle,k, -1, 1, 1, 0.01,0,1.0);
            servos[BJ_m] = servo;
            backboneServo = servo;
        }else{
        ...
        for (int i =0; i<LEG_POS_MAX; i++)
        {
            ...
            const double lr =
                    (leg==L0||leg==L1||leg==L2)-(leg==R0||leg==R1||leg==R2);
        ...
        for ( int i = 0; i < LEG_POS_MAX; i++ )
        {
            ...
            if (legPosUsage[leg]==LEG)
            {
                ...
                const double pmrl =
                        (leg==R0||leg==R1||leg==R2)-(leg==L0||leg==L1||leg==L2);
                ...
                OneAxisServo * servo1 = new OneAxisServoVel(
                        odeHandle,j, -1, 1, 1, 0.01,0,1.0);
                ...
                HingeJoint* k = new HingeJoint(
                        coxaThorax, secondThorax, anchor2, -axis2);
                k->init(odeHandle, osgHandleJoint, true, t1 * 2.1);
                legs[leg].ctrJoint = k;
                joints.push_back(k);
                /** parameters are set later */
                OneAxisServo * servo2 = new OneAxisServoVel(
                        odeHandle,k, -1, 1, 1, 0.01,0,1.0);
                //PUSH THIS STUFF BACK INTO STH!!!! not hipservos obviously
                ...
                // springy knee joint
                HingeJoint* l = new HingeJoint(
                        secondThorax, tebia, anchor3, -axis3);
                ...
                /** parameters are set later */
                OneAxisServo * servo3 =
                        new OneAxisServoVel(odeHandle,l, -1, 1, 1, 0.01,0,1.0);
            ...
            else if (legPosUsage[leg] == WHEEL)
            {
                ...
                anchor -= Pos(0,0,0);
                HingeJoint * wheeljoint = new HingeJoint(
                        objects[0],
                        wheel,
                        anchor,
                        Axis(0,1,0)*trunkPos);
    ...
    bool AmosII::setParam(const paramkey& key, paramval val){
        ...
        for (LegMap::iterator it = legs.begin(); it != legs.end(); it++)
        {
            ...
            if (footspring)
            {
                ...
                footspring->setMinMax(
                        conf.footSpringLimitD,
                        conf.footSpringLimitU);
            }
    ...
    {
        if (leg==L0 && joint==TC)  return TL0_m;
        if (leg==L0 && joint==CTR) return CL0_m;
        if (leg==L0 && joint==FTI) return FL0_m;
        if (leg==L1 && joint==TC)  return TL1_m;
        if (leg==L1 && joint==CTR) return CL1_m;
        if (leg==L1 && joint==FTI) return FL1_m;
        if (leg==L2 && joint==TC)  return TL2_m;
        if (leg==L2 && joint==CTR) return CL2_m;
        if (leg==L2 && joint==FTI) return FL2_m;
        if (leg==R0 && joint==TC)  return TR0_m;
        if (leg==R0 && joint==CTR) return CR0_m;
        if (leg==R0 && joint==FTI) return FR0_m;
        if (leg==R1 && joint==TC)  return TR1_m;
        if (leg==R1 && joint==CTR) return CR1_m;
        if (leg==R1 && joint==FTI) return FR1_m;
        if (leg==R2 && joint==TC)  return TR2_m;
        if (leg==R2 && joint==CTR) return CR2_m;
        if (leg==R2 && joint==FTI) return FR2_m;
        return AMOSII_MOTOR_MAX;
    }
    ...
    AmosII::LegJointType AmosII::getLegJointType(MotorName name)
    {
        ...
        switch(name)
        {
        case TR0_m: case TR1_m: case TR2_m: case TL0_m: case TL1_m: case TL2_m:
            return TC;
        case CR0_m: case CR1_m: case CR2_m: case CL0_m: case CL1_m: case CL2_m:
            return CTR;
        case FR0_m: case FR1_m: case FR2_m: case FL0_m: case FL1_m: case FL2_m:
            return FTI;
        default:
    ...
    AmosII::LegPos AmosII::getMotorLegPos(MotorName name)
    {
        ...
        switch(name)
        {
        case TR0_m: case CR0_m: case FR0_m: return R0;
        case TR1_m: case CR1_m: case FR1_m: return R1;
        case TR2_m: case CR2_m: case FR2_m: return R2;
        case TL0_m: case CL0_m: case FL0_m: return L0;
        case TL1_m: case CL1_m: case FL1_m: return L1;
        case TL2_m: case CL2_m: case FL2_m: return L2;
        default:
    ...
    AmosIIConf AmosII::getExperimentConf(
            double _scale,
            bool _useShoulder,
            bool _useFoot,
            bool _useBack)
    ...
    AmosIIConf AmosII::getFixedConf(
            double _scale,
            bool _useShoulder,
            bool _useFoot,
            bool _useBack)
    ...
    AmosIIConf AmosII::getFranksDefaultConf(
            double _scale,
            bool _useShoulder,
            bool _useFoot,
            bool _useBack)
