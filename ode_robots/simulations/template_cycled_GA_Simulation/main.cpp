...
public:
  ...
  void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global)
  {
	...
	// - a GenerationSizeStrategy here we take a fixed size strategy. This mean every generation have the size of "numberIndividuals"
	...
	// inverted fitness strategy because the genetic algorithm will optimize again zero. more details to this strategies in there
	// header files
	...
	//      - the prototypes need a name, a random stradegy to create random gens and a mutation strategy to mutate existing gens
	...
	// how much will die on the end and if he should make an automatically update of the values.
	// the automatically update isn't possible because we need before a run of the simulation. so we make it later self!
	SingletonGenAlgAPI::getInstance()->prepare(numberIndividuals,numberIndividuals/2,false);

	// so we are ready to start the algorithm! Need is only the simulation!
...
private:
	...
	void createBots(GlobalData& global) {
		...
		for(int ind=0;ind<numberIndividuals;ind++) {
			// fist we need the individual from the genetic algorithm because there gens say us which values are inside the neuron
			// matrix of the robot!
