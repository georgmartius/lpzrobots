...
TripodGait18DOF::TripodGait18DOF( const TripodGait18DOFConf& _conf)
  : AbstractController("TripodGait18DOF", "$Id: tripodgait18dof.cpp,v 0.1 $"), conf(_conf){
  t=0;  
...
void TripodGait18DOF::stepNoLearning(const sensor* x_, int number_sensors, 
  ...
  y_[FR2_m] = -y_[1];
  y_[TL2_m] =-outputH2 * conf.fact + conf.bias; //left rear coxa (knee) forward-backward joint
  y_[CL2_m] =-outputH1 * conf.fact * conf.direction;
  y_[FL2_m] = -y_[4];
  y_[TR1_m] =-outputH2 * conf.fact + conf.bias; //right middle coxa (knee) forward-backward joint
  y_[CR1_m] =-outputH1 * conf.fact * conf.direction;
  y_[FR1_m] = -y_[7];
  y_[TL1_m] = outputH2 * conf.fact + conf.bias; //left middle coxa (knee) forward-backward joint
  y_[CL1_m] = outputH1 * conf.fact * conf.direction;
  y_[FL1_m] = -y_[10];
  y_[TR0_m] = outputH2 * conf.fact + conf.bias; //right front coxa (knee) forward-backward joint
  y_[CR0_m] = outputH1 * conf.fact * conf.direction;
  y_[FR0_m] = -y_[13];
  y_[TL0_m]=-outputH2 * conf.fact + conf.bias; //left front coxa (knee) forward-backward joint
